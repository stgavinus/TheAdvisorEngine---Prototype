...
class LogicEngine:
    ...
    def next_two_years_multi(self, completed: set, program_slugs: list, max_credits: int = 21) -> list[dict]:
        """
        Generate a suggested 4-semester plan across multiple programs combined,
        respecting prerequisites and a per-semester credit limit.
        """
        remaining: set = set()
        for slug in program_slugs:
            if slug not in self.programs:
                continue
            audit = self.degree_audit(completed, slug)
            for b in audit["blocks"]:
                if not b["met"]:
                    for c in b["remaining_courses"]:
                        remaining.add(c)

        plan = []
        taken = set(completed)

        for semester_num in range(1, 5):
            available = [
                c for c in remaining
                if c not in taken
                and c in self.courses
                and _prereqs_satisfied(self.courses[c].prerequisites, taken)
            ]
            available.sort(
                key=lambda c: sum(1 for o in remaining if self._is_ancestor(c, o)),
                reverse=True,
            )
            semester = []
            credits = 0
            for code in available:
                node = self.courses.get(code)
                try:
                    c = int(node.credits.split("-")[0]) if node and node.credits else 3
                except (ValueError, AttributeError):
                    c = 3
                if credits + c <= max_credits:
                    semester.append({"code": code, "title": node.title if node else "", "credits": c})
                    credits += c
                    taken.add(code)

            plan.append({"semester": semester_num, "courses": semester, "total_credits": credits})

        return plan

    def next_two_years(self, completed: set, program_slug: str) -> list[dict]:
        """
        Generate a suggested course sequence for the next ~4 semesters,
        respecting prerequisites and a per-semester credit limit.
        This is the 'Possibility Space' from the assignment spec.
        """
        if program_slug not in self.programs:
            return []

        prog = self.programs[program_slug]
        audit = self.degree_audit(completed, program_slug)
        remaining = {
            c for b in audit["blocks"] if not b["met"]
            for c in b["remaining_courses"]
        }
        ...
        for semester_num in range(1, 5):
            ...
            # Prioritise courses that unlock the most others
            available.sort(
                key=lambda c: sum(1 for o in remaining if self._is_ancestor(c, o)),
                reverse=True,
            )
            # Fill semester up to ~15 credits
            semester = []
            credits = 0
            for code in available:
                node = self.courses.get(code)
